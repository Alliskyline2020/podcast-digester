...
cp .env.example .env
# 编辑 .env，至少填入 LLM_API_KEY（或旧名 DEEPSEEK_API_KEY）
# 想换 provider 见上方「切换示例」
```
...
- **粘贴链接后一直不动** → Worker 没起；`start.sh` 不含 Worker，需单独 `python worker.py`。
- **YouTube 抓取失败 / 超时** → 多为网络，需配代理 `HTTPS_PROXY=http://127.0.0.1:7897`（按你的代理改）。
- **B 站下载失败** → 反爬，需用浏览器登录态（cookie），见 `app/utils/` 的 cookie 解析。
- **无字幕源卡在 transcribe** → macOS 需先构建 AFM 3 桥接：`cd backend/tools && ./build_apple_asr.sh`。
