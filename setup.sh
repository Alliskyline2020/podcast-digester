...
echo "  2. 启动 API + 前端：  ./start.sh"
echo "  3. 另开终端启动 Worker（跑 Pipeline，必须单独起）："
echo "       cd backend && source venv/bin/activate && python worker.py"
echo "  4. 浏览器打开 http://localhost:5173 ，粘贴一个播客/视频链接即可"
echo
c_warn "提示：start.sh 不含 Worker；不单独启动 Worker 的话，粘贴链接后不会处理。"
