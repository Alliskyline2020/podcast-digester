"""字幕优化:加正确标点 + 去口水话,不改语义/时间戳(支持中文和英文)。

用户需求:ASR 字幕标点不准/缺失 + 口水话多
(中文"然后呢""呃对""就是说";英文 "um""uh""you know""like"),
影响可读性。本服务用 LLM 做两件事:
1. 补充正确标点(根据语义断句)
2. 去除无意义口水话/填充词/重复词

铁律:
- 不改语义(不增删信息,不修正事实,不翻译)
- 不改时间戳(start_ms/end_ms/id 完全不动)
- 只填充 text_with_punct 字段,text_original 保留作为原始备份
- 输出句数 = 输入句数,一一对应,输出语言与输入相同
"""
import json
import logging
...
1. **补充正确标点**:ASR 输出的标点经常不准、缺失或位置错误。根据语义重新加正确的标点让字幕自然可读,原标点位置错就删掉重加。
   - 中文:逗号、句号、问号、感叹号、顿号、分号
   - 英文:comma, period, question mark, exclamation mark, semicolon, colon;并修正首字母大写与句末标点

2. **去除口水话和无意义填充**,但保留有语义作用的词:
   - 中文:删除"嗯""啊""呃""那个""然后呢""就是说""对吧""你看""的话""就是这个"等口头禅、语气词、无意义重复
   - 英文:删除 "um", "uh", "er", "ah", "hmm", "you know", "I mean", "like"(填充用法), "sort of", "kind of", "basically", "actually", "literally"(填充用法), 以及无意义重复(如 "the the", "I I")
   - 中文"然后/因为"、英文 "because/therefore/however/so"(表逻辑时) → 保留
   - 人名/产品名/公司名/数字/专业术语 → 原样保留

**绝对铁律(违反任一条都算失败)**:
- 不改变语义,不增删信息,不修正事实错误,不补全省略内容
- 只优化"怎么说",不改"说什么"
- 输出语言必须与输入相同(中文输入→中文输出,英文输入→英文输出;不翻译)
- 输出句数必须严格等于输入句数,按 id 一一对应
- 每句输出对应输入同一句(不改顺序、不合并、不拆分)
- 人名、产品名、公司名、数字、术语保持原样(如 Manus、OpenAI、2025、GPT-4)
- 如果某句已经很好(无需改),原样输出"""
...
async def polish_subtitles(
    ...
    Args:
        ...
        batch_size: 每批处理多少句(默认 15,non-thinking 模式长批次 JSON 易出错)
...
async def polish_episode_subtitles(
    ...
    if language not in ("zh", "en"):
        return {"episode_id": episode_id, "skipped": True, "reason": f"language={language}, 只处理中文/英文"}
