...
# ---------- polish 跳过检测 ----------
# manual CC(尤其英文大频道)本身标点完整, polish 的"补标点"功能多余,
# 只剩"去填充词"的微小收益 — 不抵 ~26 分钟 LLM 成本(11696 段 / batch 15 = ~780 批)。
# 用"句末强标点(.!?。！？)密度"判定: manual CC 实测 ~0.012, ASR/单逗号 ~0。
# 比纯标点密度更鲁棒 — 单逗号长句总密度高(0.048)但无句末标点(0), 会误判。
POLISH_SKIP_SAMPLE = 50                # 均匀采样段数(跨全部段, 避免开场白偏差)
POLISH_SKIP_MIN_END_DENSITY = 0.005    # 句末强标点(.!?)密度阈值


def _already_punctuated(transcript) -> bool:
    """检测 transcript 是否已有完整标点 → 跳过 polish。

    均匀采样 POLISH_SKIP_SAMPLE 段拼接, 算句末强标点(.!?。！？)密度。
    密度 >= POLISH_SKIP_MIN_END_DENSITY → True(manual CC, polish 多余)。
    密度对分段方式不敏感(聚合不改变 count/chars), 对 raw segments 同样有效。
    """
    segs = transcript.segments
    if not segs:
        return False
    n = len(segs)
    step = max(1, n // POLISH_SKIP_SAMPLE)
    sampled = segs[::step][:POLISH_SKIP_SAMPLE]
    text = "".join((s.text_original or "") for s in sampled)
    if not text.strip():
        return False
    end_marks = ".!?。！？"
    end_count = sum(1 for c in text if c in end_marks)
