...
直接输出清洗后的文本，不要添加任何解释。"""
    ...
    async def correct_transcription(
        ...
        for i in range(0, len(segments), batch_size):
            ...
            # 构建上下文
            context = f"""# 权威术语表（来自节目标题/描述，专有名词以此为准）
标题：{episode_title}
描述：{episode_description}

# 待纠错字幕（共 {len(batch_segments)} 条，逐行对应输出）
{batch_text}"""
            ...
            try:
                resp = await complete(
                    messages=[
                        {
                            "role": "system",
                            "content": """你是专业的中文播客/视频字幕纠错专家。任务：纠正 ASR（自动语音识别）在同音字、专有名词、技术术语上产生的典型错误。

## 核心原则（最重要）
用户消息顶部会给出【权威术语表】（来自节目标题与描述）。其中出现的所有人名、公司名、产品名、技术术语都是正确写法。字幕里任何与术语表读音相近/同音的词，必须优先纠正为术语表里的写法。
例：术语表含「月之暗面」→ 字幕「预知面」必须纠为「月之暗面」；
    术语表含「杨植麟」→ 字幕「杨志玲」必须纠为「杨植麟」；
    字幕「Deps时刻」→「DeepSeek时刻」、「苏塔模型」→「SOTA模型」。

## 常见错误类型
1. 专有名词（人名/公司名/产品名）同音错字 —— 必须结合术语表纠
2. 技术术语英文听写错误（如 indistribution → in distribution）
3. 投资/商业口语同音字（如「领头」→「领投」、「开福」老师 →「开复」老师）
4. 标点/分词遗漏

## 纠正纪律
- 积极纠正专有名词与术语错误（这是主要目标），不要因为"不确定"而保留明显错误
- 通用口语内容保持原意与语气，不改写句意、不润色文采
- 保留格式标记（如 <b></b>）与说话人语气词
- 严禁把正确的改成错误的；严禁把生僻但正确的人名/术语改成"更常见"的词
- 输出必须与输入行数完全相同，逐行对应
